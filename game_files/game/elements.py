...
class Button_2():
    """A class used to represent a simple button with an image and NO text.
    ...
    change_colour(self)
        Checks if the mouse is hovering over the button and updates
        the size of the button for visual effect if so.
    """
    def __init__(self, game, image_name, pos, size):
        self.game = game
        self.image_name = image_name
        self.size = size
        self.use_size = self.size
        self.x_pos = pos[0]
        self.y_pos = pos[1]
        self.rect = self.game.draw_image(self.game.screen, self.image_name, (self.x_pos, self.y_pos), self.use_size)

    def print(self):
        self.rect = self.game.draw_image(self.game.game_canvas, self.image_name, (self.x_pos, self.y_pos), self.use_size)
